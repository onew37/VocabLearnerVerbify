...
const STREAK_BENCHMARKS = [3, 7, 14, 30, 50, 75, 100, 150, 200, 250, 300, 365];
function isStreakBenchmark(s) { return STREAK_BENCHMARKS.indexOf(s) >= 0 || (s > 365 && s % 100 === 0); }
...
function defaultState() {
  return {
    ...
    settings: { audio: true, voice: null, theme: "notebook", seasonal: true, casual: false, testPeriodDays: 7, variants: { terminal: "green", aesthetic: "ocean", neon: "synthwave" } },
  };
}
...
/* Celebration when the daily streak hits a benchmark — full-screen fireworks + a pop-in badge. */
function streakMilestoneSub(s) {
  if (s >= 365) return "A full year — legendary dedication!";
  if (s >= 200) return "Two hundred days. Unstoppable.";
  if (s >= 100) return "Triple digits — you're a vocabulary machine!";
  if (s >= 50) return "Fifty days strong — incredible consistency.";
  if (s >= 30) return "A whole month! This is a real habit now.";
  if (s >= 14) return "Two weeks running — momentum is building.";
  if (s >= 7) return "A full week! Keep the fire going.";
  return "Three days in a row — nice start!";
}
function showStreakMilestone(streak) {
  const el = document.createElement("div");
  el.className = "streak-milestone";
  el.innerHTML = `<div class="sm-inner">
      <div class="sm-flame">${ICON.flame("3em")}</div>
      <div class="sm-num">${streak}</div>
      <div class="sm-label">DAY STREAK!</div>
      <div class="sm-sub">${streakMilestoneSub(streak)}</div>
    </div>`;
  document.body.appendChild(el);
  if (window.FX) FX.start("fireworks");
  requestAnimationFrame(() => el.classList.add("show"));
  setTimeout(() => el.classList.remove("show"), 4200);
  setTimeout(() => { if (el.parentNode) el.parentNode.removeChild(el); applySeasonalFX(effectiveTheme()); }, 4900);
}
...
function eligiblePool(level) {
  // Casual mode isn't test-graded, so don't gate by the narrow skill window — serve the whole
  // everyday pool (capped below the most obscure tier) so daily-use words flow from day one.
  if (casualMode()) return activeWords().filter(w => wordDifficulty(w.word) <= 4);
  const { lo, hi } = difficultyWindow(level);
  return activeWords().filter(w => { const d = wordDifficulty(w.word); return d >= lo && d <= hi; });
}
...
function pickDailyWords(n) {
  ...
  // 1) fresh words from the eligible pool. Academic tiers by skill difficulty; casual (not
  //    test-graded) just draws a random everyday mix so daily-use words flow from day one.
  const pool = fresh(eligiblePool(level));
  if (casualMode()) {
    shuffleRandom(pool).forEach(add);
  } else {
    shuffleRandom(pool.filter(w => wordDifficulty(w.word) === level)).forEach(add);
    shuffleRandom(pool.filter(w => wordDifficulty(w.word) !== level)).forEach(add);
  }
  // 2) any fresh word in this learner's dictionary (casual vs academic pool).
  if (chosen.length < n) shuffleRandom(fresh(activeWords())).forEach(add);
  // 3) pool exhausted (rare): allow anything in the pool so the review still works.
  if (chosen.length < n) shuffleRandom(activeWords()).forEach(add);
...
function casualMode() { return !!(state.settings && state.settings.casual); }
/* The active vocabulary pool for this learner: casual learners use the everyday-use dictionary
   (pool "casual"); academic learners use the vetted SSAT dictionary (everything not tagged casual).
   Falls back to academic until the casual pack has loaded. */
function activeWords() {
  if (casualMode()) {
    const c = WORDS.filter(w => w.pool === "casual" || w.pool === "both");
    if (c.length >= 50) return c;
    return WORDS.filter(w => w.pool !== "casual"); // casual dictionary not ready yet → academic
  }
  return WORDS.filter(w => w.pool !== "casual"); // academic = academic + shared("both") + originals
}
function testDue() {
  if (casualMode()) return false;                            // casual mode → no mandatory tests, ever
...
/* Permanently delete the signed-in account and its saved progress, then sign out. */
function deleteCurrentAccount() {
  const key = currentUser;
  const acct = loadAccounts()[key];
  const who = acct ? "@" + acct.username : "this account";
  if (!window.confirm(`Permanently delete ${who} and ALL of its progress on this device? This can't be undone.`)) return;
  const accounts = loadAccounts();
  delete accounts[key];
  saveAccounts(accounts);
  localStorage.removeItem("lexicon.state.v1::" + key);   // that account's saved progress
  if (getSession() === key) setSession(null);
  stopExamTimer();
  session = null; quiz = null; match = null;
...
/* ============================================================
   ...
   ============================================================ */
let chosenGoal = 5;
let chosenCasual = false;    // casual mode (no mandatory tests) chosen at signup
...
function renderDiagnostic() {
  ...
  if (!body) return;
  // Start genuinely easy: the lower tiers are common everyday words (not the academic
  // dictionary), so a casual learner sees familiar words. Only the top tier is test-prep vocab.
  const EASY = ["happy","brave","calm","honest","gentle","clever","polite","eager","fierce","sturdy","vivid","swift","gather","fragile","curious","silent","clumsy","gigantic","gloomy","weary","bold","tidy","rapid","simple"];
  const MEDIUM = ["generous","stubborn","ancient","anxious","awkward","cautious","cheerful","delicate","gracious","humble","jealous","loyal","modest","peculiar","reckless","sincere","timid","witty","abrupt","decent","frantic","vague","blunt","obvious"];
  const HARD = WORDS.filter(w => DIFFICULTY[w.word.toLowerCase()] && wordDifficulty(w.word) >= 4).map(w => w.word);
  const sample = (arr, n) => shuffleRandom(arr.slice()).slice(0, n);
  diagShown = [
    { key: "beg", name: "Everyday words",  words: sample(EASY, 8) },
    { key: "int", name: "A little harder", words: sample(MEDIUM, 8) },
    { key: "adv", name: "Challenging",     words: sample(HARD, 8) },
  ];
  body.innerHTML = diagShown.map(g => `
    <div class="diag-group">
      <div class="diag-h">${g.name}</div>
      <div class="diag-chips">
        ${g.words.map(w => `<button type="button" class="diag-chip" data-word="${escapeHtml(w)}">${escapeHtml(w)}</button>`).join("")}
      </div>
    </div>`).join("");
...
function diagnosticLevel() {
  const ratio = (band) => {
    ...
    return g.words.filter(w => diagKnown.has(w)).length / g.words.length;
  };
...
function finishOnboarding(startLevel) {
  ...
  state.settings.casual = chosenCasual;
  save();
  document.getElementById("onboarding").classList.add("hidden");
...
function setupOnboarding() {
  ...
  document.querySelectorAll("#pathOptions .path-card").forEach(card => {
    card.addEventListener("click", () => {
      document.querySelectorAll("#pathOptions .path-card").forEach(c => c.classList.remove("selected"));
      card.classList.add("selected");
      chosenCasual = card.dataset.casual === "1";
    });
  });
  // Step 1 (path + goal) → Step 2 (placement diagnostic).
...
function showOnboardingOrApp() {
  ...
  if (state.onboarded) {
    ...
    chosenCasual = false; // default to Academic
    document.querySelectorAll("#pathOptions .path-card")
      .forEach(c => c.classList.toggle("selected", c.dataset.casual === "0"));
  }
}
...
function getTodayWords() {
  ...
  // Reuse today's already-chosen set (stable across reloads/re-logins on the same day) — but only
  // if it was built for the SAME learner mode. Switching casual↔academic rebuilds from the new pool.
  if (ds && ds.date === t && (ds.casual || false) === casualMode() && Array.isArray(ds.words) && ds.words.length) {
    ...
    if (ws.length) {
      if (!locked && ws.length < n) {            // goal increased → add fresh NEW words only
      ...
      } else if (!locked && ws.length > n) {
        ws = ws.slice(0, n);                       // goal decreased
      }
      const cleaned = ws.map(w => w.word);
      if (cleaned.join("|") !== ds.words.join("|")) { // persist the repaired set
        state.dailySet = { date: t, words: cleaned, skill: (ds.skill != null ? ds.skill : skillLevel()), casual: casualMode() };
        save();
      }
      return ws;
    }
  }
  const chosen = buildDailyMix(n);
  state.dailySet = { date: t, words: chosen.map(w => w.word), skill: skillLevel(), casual: casualMode() };
  save();
  return chosen;
}

/* NEW words come first and fill the daily goal — you're never served words you already know
   while fresh ones remain. Spaced-repetition reviews of due words only top up the set once
   you've genuinely run out of new words to learn (then the mandatory test / dictionary carry SRS). */
function buildDailyMix(n) {
  const fresh = pickDailyWords(n);
  if (fresh.length >= n) return dedupWords(fresh).slice(0, n);   // enough new words → all new
  const have = new Set(fresh.map(w => w.word));
  const due = dueWords().map(s => WORDS.find(w => w.word === s)).filter(Boolean).filter(w => !have.has(w.word));
  return dedupWords(fresh.concat(due)).slice(0, n);
}
...
function renderFlashcard() {
  ...
  const isReview = !!statOf(w.word); // studied in a prior session → a review, not a brand-new word
  ...
  stage.innerHTML = `
    <div class="review-wrap">
      ...
      <div class="flashcard${isReview ? " is-review" : ""}" id="flashcard">
        ${isReview ? `<div class="fc-watermark" aria-hidden="true">REVIEW</div>` : ""}
        <div class="fc-topline">${isReview ? `<span class="fc-review-badge">${ICON.refresh("0.85em")} Review</span>` : ""}<span class="fc-pos">${w.pos}</span>${diffBadge(w.word)}</div>
...
function renderReveal(w) {
  ...
  return `
    <div class="fc-reveal">
      ...
      ${(rootPills || w.formation) ? `<div class="fc-block"><h4>Word formation &amp; roots</h4>
...
function buildQuizQuestions() {
  ...
  // Quick Quiz always mixes formats; Academic SAT mode also brings variety into the
  // daily review (fill-in-the-blank, type-the-word). SSAT/ISEE stay the clean synonym format.
...
function finishSession() {
  ...
  if (!alreadyDoneToday) {
    ...
    completeToday();
    streakBenchmark = isStreakBenchmark(state.streak);
  ...
  if (streakBenchmark) showStreakMilestone(state.streak);   // celebrate streak benchmarks
...
function renderPractice() {
  ...
  const cards = [
    ...
    { icon: ICON.clock("30px"), title: "Timed Exam", color: "red", desc: "A quick simulation: 10 questions against the clock (SSAT / ISEE / SAT style).", btn: "Begin timed test", act: "startExam", disabled: false, casualHidden: true },
  ].filter(c => !(casualMode() && c.casualHidden));   // casual learners don't get test modes
  body.innerHTML = `
...
function renderSettings() {
  ...
  body.innerHTML = `
    ...
    <div class="card prog-card">
      <h3>Learning mode ${ICON.brain("0.9em")}</h3>
      <label class="set-row toggle-row"><span>Casual learner ${ICON.bulb("1em")} <span class="muted small">— just learn useful everyday words, no mandatory tests</span></span>
        <input type="checkbox" id="setCasual" ${casual ? "checked" : ""} /></label>
      <div class="set-row" id="testPeriodRow" style="${casual ? "opacity:.4;pointer-events:none;" : ""}"><span class="muted">${ICON.trophy("0.9em")} Academic — test me every</span>
        <select id="setTestPeriod">${[3, 5, 7, 10, 14, 30].map(d => `<option value="${d}" ${testPeriodDays() === d ? "selected" : ""}>${d} days</option>`).join("")}</select>
      </div>
      <p class="muted small">${casual
        ? "Casual mode is on — no exams. You'll keep learning useful everyday words at your own pace (the most obscure test-only words are set aside)."
        : "Academic mode: a no-hints mastery exam over most words you've learned, on the schedule above. You must pass it to unlock new daily words."}</p>
    </div>
    ...
    <div class="card prog-card danger-card">
      <h3>Danger zone</h3>
      <p class="muted small">Reset wipes this account's streak, words, XP and stats. Delete removes the account entirely. Neither can be undone.</p>
      <div class="danger-actions">
        <button class="btn-ghost sm danger" id="resetBtn">Reset all progress</button>
        <button class="btn-ghost sm danger" id="deleteAcctBtn">Delete account</button>
      </div>
    </div>
  ...
  document.getElementById("setCasual").addEventListener("change", e => {
    state.settings = state.settings || {};
    state.settings.casual = e.target.checked;
    state.dailySet = null;       // rebuild today's words from the new pool (casual vs academic)
    state.review = null;         // abandon any in-progress review/test so it rebuilds for the new mode
    session = null; quiz = null;
    save();
    renderSettings();
  });
  ...
  document.getElementById("deleteAcctBtn").addEventListener("click", deleteCurrentAccount);
...
function renderDictionary() {
  ...
  if (statsEl) {
    const nCasual = WORDS.filter(w => w.pool === "casual").length;
    const nAcademic = WORDS.length - nCasual;
    const nActive = activeWords().length;
    const nAna = (window.ANALOGIES || []).length;
    const rseen = new Set();
    Object.values(ROOTS).forEach(r => rseen.add(r.root + "|" + r.meaning));
    statsEl.innerHTML =
      `<span class="dstat" title="Words in your current dictionary"><b>${nActive.toLocaleString()}</b> ${casualMode() ? "casual" : "academic"} words</span>` +
      `<span class="dstat">${ICON.brain("0.85em")} <b>${nAcademic.toLocaleString()}</b> academic</span>` +
      `<span class="dstat">${ICON.bulb("0.85em")} <b>${nCasual.toLocaleString()}</b> casual</span>` +
      `<span class="dstat"><b>${nAna.toLocaleString()}</b> analogies</span>` +
      `<span class="dstat"><b>${rseen.size.toLocaleString()}</b> roots</span>`;
  }
